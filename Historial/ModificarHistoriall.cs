...
namespace Roll_Call.Historial
{
    public partial class ModificarHistoriall : Form
    {
        ...
        {
            InitializeComponent();
            ConfigurarDataGridView(); // Configura el DataGridView al iniciar el formulario
        ...
        private void CargarDatos()
        {
            try
            {
                using (SqlConnection conexion = new SqlConnection(DAL.conexion.CadenaConexion))
                {
                    conexion.Open();
        ...
        {
            if (!string.IsNullOrEmpty(cbxMateria.Text))
            {
                BuscarAsistencia();
            }
        }
